...
class thread_pool_win : public thread_pool
{
  ...
  */
  ...
  class native_timer : public timer
  {
    ...
    {
      /*
       According to documentation, Windows timers are properly synchronizing
       (i.e only one callback would run). But it seems not to be always the case
       thus m_running is taking case of proper synchronization.
      */
      if (++m_running == 1)
      {
        m_task.execute();
        if (m_period)
          set_time(m_period, m_period);
      }
      --m_running;
    }
  ...
  public:
    native_timer(thread_pool_win& pool, callback_func func, void* data, task_group* group):
          m_mtx(), m_task(func, data, group), m_pool(pool), m_period(), m_on(true),m_running(0)
    {
    ...
    /* Cancel pending and future callbacks, wait running ones until they are finished. */
