...
public:
  ...
  public:
    timer_generic(callback_func func, void* data, task_group *group, thread_pool_generic * pool):
      ...
      m_on(true),m_running(false)
    {
      if (pool)
      {
        /* Execute callback in threadpool*/
        thr_timer_init(this, submit, this);
      }
      else
      {
        /* Execute callback in "timer" thread */
    ...
    void set_time(int initial_delay_ms, int period_ms) override
    {
      ...
      /* Need to end pending timer before reset (thr_timer is weird) */
      thr_timer_end(this);

      /* Handle periodic timers,*/
      assert(!period || !m_task.m_group);
      if (!m_pool)
      {
      ...
      /*
        Optimization for timer with zero delay - do not use thr_timer.
        Use thread_pool::submit_task() instead.
      */
      if (!initial_delay_ms && !m_running && m_pool)
      {
        /* Cancel pending tasks, if any.*/
        if (m_task.pending_or_running())
        {
          if (m_task.m_group)
  ...
  m_maintaince_timer_task()
{
  ...
  if (min_threads < max_threads)
  {
    ...
    m_maintaince_timer_task.reset(new timer_generic(thread_pool_generic::maintainence_func, this, nullptr, nullptr));
