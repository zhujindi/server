...
{
	return m_coordinator_state.m_sigcount != 0;
}

/**  Change purge coordinator state to idle */
void purge_sys_t::set_idle()
{
	m_coordinator_state.m_sigcount = 0;
}

/**  Change purge coordinator state to idle */
void purge_sys_t::set_running()
{
	m_coordinator_state.m_sigcount = 1;
}
...
srv_wake_purge_thread_if_not_active()
{
	...
	if (purge_sys.enabled() && !purge_sys.paused()
		...
		if(++purge_sys.m_coordinator_state.m_sigcount == 1) {
			purge_coordinator_timer->set_time(0,0);
		}
	}
}
...
extern "C" void thd_detach_thd(void *);
...
void purge_coordinator()
{
	...
	purge_sys.m_coordinator_state.m_history_length = 0;
	...
	do {
		...
		int32_t sigcount = purge_sys.m_coordinator_state.m_sigcount;

		purge_sys.m_coordinator_state.m_history_length = srv_do_purge(&n_total_purged);

		/* Check if there were some srv_wake_purge_thread_if_not_active() while we worked.*/

		bool woken_during_purge = purge_sys.m_coordinator_state.m_sigcount != sigcount;
...
void purge_coordinator_timer_callback(void *)
{
	...
	if (!purge_sys.running() // executed by idle timeout
		&& trx_sys.rseg_history_len < 5000
		&& purge_sys.m_coordinator_state.m_history_length
				== trx_sys.rseg_history_len) {
	...
	/* Prevent wakeups, while purge is already running.*/
	purge_sys.set_running();
...
void srv_shutdown_purge_tasks()
{
	...
	ut_a(!purge_worker_task.pending_or_running());
	purge_worker_task.wait();
	assert(!purge_thds.empty());
...
srv_purge_wakeup()
{
	...
	purge_sys.m_coordinator_state.m_sigcount = 0;
	srv_wake_purge_thread_if_not_active();
}
