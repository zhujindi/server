...
void purge_sys_t::create()
{
  ...
  truncate.last= NULL;
  m_coordinator_state.m_history_length = 0;
...
extern tpool::timer *purge_coordinator_timer;
...
void purge_sys_t::stop()
{
  ...
  if (m_paused++ == 0)
  {
    ...
    purge_coordinator_timer->disable();
    /* Signal that we're not running.*/
    m_coordinator_state.m_sigcount = 0;
...
void purge_sys_t::resume()
{
   ...
   if (paused == 1)
   {
     purge_coordinator_timer->enable();
